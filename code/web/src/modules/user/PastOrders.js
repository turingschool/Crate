import React, { PureComponent } from 'react'
import PropTypes from 'prop-types'
import { Grid, GridCell } from '../../ui/grid'
import { H2, H6 } from '../../ui/typography'
import { grey, grey4, secondary } from '../../ui/common/colors'

class PastOrders extends PureComponent {

    componentDidMount() {

    }

    render() {
        return (
            <GridCell style={{ backgroundColor: secondary, height: "100%"}}>
                <H2>My Past Orders</H2>
                <Grid style={{ backgroundColor: grey, margin: "3em", padding: '.5em' }}>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                </Grid>
                <Grid style={{ backgroundColor: grey, margin: "3em", padding: '.5em' }}>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                    <GridCell style={{ textAlign: "center"}}>
                        <img 
                            src="https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcTmxYkmH9BRcMKj8QFHWz8AU92943C0djkzylgdY8N3ElCVq2S6tqJoX0dTw5F7M9KCAmKMGA2W84oBwLpbnG47OBAFIqOso4OkX4T7lAs&usqp=CAc"
                            style={{ width: "125px"}}
                        />
                        <H6>Jeans</H6>
                        <p>You Kept These!</p>
                    </GridCell>
                </Grid>
            </GridCell>
        )
    }
}

export default PastOrders